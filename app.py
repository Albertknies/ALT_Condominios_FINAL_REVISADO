...
FIELDS = [
    ...
    "possuiInternet", "operadoraInternet", "loginInternet", "senhaInternet",
    "sabeConstrutora", "nomeConstrutora", "cnpjConstrutora", "responsavelConstrutora",
    "contatoConstrutora", "entregaConstrutora", "predioGarantia", "garantiaAte",
    "obs"
]
YES_NO = {"Sim", "Não"}
...
def collect_form(form):
    ...
    for key in ("acessoSenha", "possuiControlePortao", "possuiInternet", "sabeConstrutora", "predioGarantia"):
        d[key] = valid_choice(d[key], YES_NO)

    d["qtdControlesPortao"] = parse_int(d["qtdControlesPortao"], 0, 100, blank="")
    for key in ("entregaConstrutora", "garantiaAte"):
        d[key] = valid_date(d[key])

    d["controlesPortao"] = []
    qtd_controles = int(d["qtdControlesPortao"] or 0) if d["possuiControlePortao"] == "Sim" else 0
    for i in range(1, qtd_controles + 1):
        codigo = clean(form.get(f"codigoControlePortao_{i}", ""), 100)
        portao = clean(form.get(f"portaoControlePortao_{i}", ""), 200)
        if codigo or portao:
    ...
    if d["acessoSenha"] != "Sim":
        d["senhasAcesso"] = ""
    if d["possuiControlePortao"] != "Sim":
        d["qtdControlesPortao"] = ""
        d["controlesPortao"] = []
    if d["possuiInternet"] != "Sim":
        for key in ("operadoraInternet", "loginInternet", "senhaInternet"):
            d[key] = ""
    if d["sabeConstrutora"] != "Sim":
        for key in ("nomeConstrutora", "cnpjConstrutora", "responsavelConstrutora", "contatoConstrutora", "entregaConstrutora", "predioGarantia", "garantiaAte"):
            d[key] = ""
    elif d["predioGarantia"] != "Sim":
        d["garantiaAte"] = ""
...
ALT_PURPLE = colors.HexColor("#4F2C68")
ALT_PURPLE_DARK = colors.HexColor("#321B45")
ALT_LAVENDER = colors.HexColor("#F0ECF5")
ALT_ROW = colors.HexColor("#F7F5FA")
ALT_LINE = colors.HexColor("#DCD6E5")
ALT_GRAY = colors.HexColor("#706B78")
ALT_LOGO = BASE / "alt_logo_trimmed.png"


def _prepare_alt_logo():
    source = BASE / "novo.png"
    if not source.exists():
        source = Path("/mnt/data/novo.png")
    if source.exists() and not ALT_LOGO.exists():
        im = PILImage.open(source).convert("RGBA")
        bbox = im.getbbox()
        if bbox:
            im.crop(bbox).save(ALT_LOGO)
    return ALT_LOGO if ALT_LOGO.exists() else None


def add_section(story,title,rows):
    clean_title = str(title)
    match = re.match(r"^(\d+)[.)]?\s*[-–—:]?\s*(.*)$", clean_title)
    number = match.group(1) if match else ""
    heading = match.group(2) if match else clean_title
    if number:
        heading_table = Table([[Paragraph(number, styles["SectionNumber"]), Paragraph(pdf_value(heading), styles["Section"]) ]], colWidths=[34, 491])
        heading_table.setStyle(TableStyle([
            ("BACKGROUND", (0,0), (0,0), ALT_LAVENDER), ("BOX", (0,0), (0,0), 0.4, ALT_LINE),
        ...
        story.append(Paragraph(pdf_value(heading), styles["Section"]))
    data=[]
    for label,value in rows:
        if value not in ("",None,[],{}):
            data.append([Paragraph(pdf_value(label),styles["Label"]),Paragraph(pdf_value(value),styles["Value"])])
    if data:
        t=Table(data,colWidths=[175,350])
        cmds=[("VALIGN",(0,0),(-1,-1),"TOP"),("GRID",(0,0),(-1,-1),0.35,ALT_LINE),
              ("LEFTPADDING",(0,0),(-1,-1),7),("RIGHTPADDING",(0,0),(-1,-1),7),
              ("TOPPADDING",(0,0),(-1,-1),5),("BOTTOMPADDING",(0,0),(-1,-1),5)]
        for i in range(len(data)):
            cmds.append(("BACKGROUND",(0,i),(-1,i), ALT_ROW if i % 2 == 0 else colors.white))
        t.setStyle(TableStyle(cmds)); story.append(t); story.append(Spacer(1,8))


styles=getSampleStyleSheet()
styles.add(ParagraphStyle(name="TitleALT",parent=styles["Title"],fontName="Helvetica-Bold",fontSize=17,textColor=ALT_PURPLE_DARK,alignment=TA_CENTER,spaceAfter=4))
styles.add(ParagraphStyle(name="SubALT",parent=styles["Normal"],fontSize=9,textColor=ALT_GRAY,alignment=TA_CENTER,spaceAfter=15))
styles.add(ParagraphStyle(name="Section",parent=styles["Heading2"],fontName="Helvetica-Bold",fontSize=12,textColor=ALT_PURPLE_DARK,spaceBefore=10,spaceAfter=6))
styles.add(ParagraphStyle(name="SectionNumber",parent=styles["Normal"],fontName="Helvetica-Bold",fontSize=10,textColor=ALT_PURPLE_DARK,alignment=TA_CENTER))
styles.add(ParagraphStyle(name="Label",parent=styles["Normal"],fontName="Helvetica-Bold",fontSize=8,textColor=ALT_PURPLE_DARK))
styles.add(ParagraphStyle(name="Value",parent=styles["Normal"],fontSize=8.5,textColor=colors.HexColor("#25212A")))
...
def pdf(cid):
    with conn() as c:
        row = c.execute("SELECT * FROM condominios WHERE id=?", (cid,)).fetchone()
    if not row:
        return "Condomínio não encontrado", 404

    d = load_data(row)
    name = clean(d.get("nome"), 200) or "Condomínio"
    now = datetime.now()
    folder = PDFS / safe_name(name) / now.strftime("%Y-%m-%d")
    folder.mkdir(parents=True, exist_ok=True)
    filename = f"{safe_name(name)}-Atualizacao-{now:%Y-%m-%d_%H-%M-%S}.pdf"
    path = folder / filename
    ...
    try:
        date_text = datetime.fromisoformat(row["atualizado_em"]).strftime("%d/%m/%Y")
    except (ValueError, TypeError):
        date_text = now.strftime("%d/%m/%Y")

    story = [
        Paragraph("ALT GESTÃO DE CONDOMÍNIOS", styles["TitleALT"]),
        Paragraph("FICHA CADASTRAL E ATUALIZAÇÃO", styles["SubALT"]),
        Paragraph(
            f"<b>Condomínio:</b> {pdf_value(name)} &nbsp;&nbsp; "
            f"<b>Data da atualização:</b> {date_text}",
            styles["Value"],
        ),
        Spacer(1, 10),
    ]

    add_section(story, "1. DADOS DO CONDOMÍNIO", [
        ("Nome completo", d.get("nome")),
        ("CNPJ", d.get("cnpj")),
        ("Endereço", address_text(d.get("endereco"))),
        ("Quantidade de unidades", d.get("unidades")),
    ])
    ...
    if d.get("sindicoTipo") == "Profissional":
        sindico_rows += [
            ("Empresa", d.get("empresaSindico")),
            ("CNPJ da empresa", d.get("cnpjEmpresaSindico")),
            ("Síndico responsável", d.get("responsavelSindico")),
            ("Telefone do responsável", d.get("telResponsavelSindico")),
        ]
    else:
        sindico_rows += [
            ("Nome completo", d.get("sindico")),
            ("CPF", d.get("cpfSindico")),
            ("Unidade/Apartamento", d.get("unSindico")),
            ("Telefone", d.get("telSindico")),
            ("E-mail", d.get("emailSindico")),
        ]
    sindico_rows += [
        ("Data da ata de eleição", format_date(d.get("ataEleicao"))),
        ("Última Assembleia Geral Ordinária (AGO)", format_date(d.get("ultAGO"))),
        ("Início do mandato", format_date(d.get("inicioMandato"))),
        ("Vencimento do mandato", format_date(d.get("fimMandato"))),
    ]
    add_section(story, "2. SÍNDICO E MANDATO", sindico_rows)

    council = [
        ("Possui Conselho Fiscal", yn(d.get("conselho"))),
        ("Quantidade de conselheiros", d.get("qtdConselheiros")),
    ]
    for i, item in enumerate(d.get("conselheiros", []), 1):
        council += [
            (f"Conselheiro {i} — nome", item.get("nome")),
            (f"Conselheiro {i} — unidade", item.get("unidade")),
        ]
    add_section(story, "3. CONSELHO FISCAL", council)

    add_section(story, "4. DADOS BANCÁRIOS", [
        ("Banco", d.get("banco")),
        ("Agência", d.get("agencia")),
        ("Conta bancária", d.get("conta")),
    ])

    add_section(story, "5. DOCUMENTAÇÃO", [
        ("Possui Convenção", yn(d.get("conv"))),
        ("Convenção impressa na pasta física", yn(d.get("convFis"))),
        ("Possui Regimento Interno", yn(d.get("reg"))),
        ("Regimento Interno impresso na pasta física", yn(d.get("regFis"))),
        ("Contrato da administradora impresso", yn(d.get("admFis"))),
    ])

    add_section(story, "6. PPCI E SEGURANÇA CONTRA INCÊNDIO", [
        ("Possui PPCI", yn(d.get("ppci"))),
        ("Validade do PPCI", format_date(d.get("validPpci"))),
        ("PPCI impresso", yn(d.get("ppciFis"))),
    ...
    add_section(story, "8. SEGURO PREDIAL", [
        ("Possui seguro predial", yn(d.get("seg"))),
        ("Empresa/Seguradora", d.get("segEmpresa")),
    ...
    add_section(story, "9. CONTAS DE CONSUMO", [
        ("Código da conta de luz", d.get("luz")),
        ("Código da conta de água", d.get("agua")),
        ("Possui gás", yn(d.get("gas"))),
        ("Responsável pela leitura/gestão do gás", d.get("gasTipo")),
    ...
    add_section(story, "12. JURÍDICO / COBRANÇA", [
        ...
        ("Nome do advogado/escritório", d.get("nomeJuridico")),
        ("Percentual aplicado nas cobranças", d.get("percCobranca")),
        ("Contrato jurídico impresso", yn(d.get("jurFis"))),
    ])

    add_section(story, "13. MERCADINHO", [
        ("Possui mercadinho", yn(d.get("mercadinho"))),
        ("Nome do mercadinho", d.get("nomeMerc")),
        ("Responsável", d.get("respMerc")),
        ("Contato", d.get("contatoMerc")),
        ("Repasse para o condomínio", d.get("repasseMerc")),
        ("Periodicidade do repasse", d.get("periodoMerc")),
        ("Data/período do repasse", d.get("dataMerc")),
        ("Contrato impresso", yn(d.get("mercFis"))),
    ])

    add_section(story, "14. INFORMAÇÕES DE ACESSO E OPERAÇÃO", [
        ("Possui acesso por senha", yn(d.get("acessoSenha"))),
        ("Senhas e locais de acesso", d.get("senhasAcesso")),
        ("Possui controles de portão", yn(d.get("possuiControlePortao"))),
        ("Quantidade de controles", d.get("qtdControlesPortao")),
        ("Possui internet contratada", yn(d.get("possuiInternet"))),
        ("Operadora de internet", d.get("operadoraInternet")),
        ("Login da internet", d.get("loginInternet")),
        ("Senha da internet", d.get("senhaInternet")),
        ("Possui informações da construtora", yn(d.get("sabeConstrutora"))),
        ("Nome da construtora", d.get("nomeConstrutora")),
        ("CNPJ da construtora", d.get("cnpjConstrutora")),
        ("Responsável da construtora", d.get("responsavelConstrutora")),
        ("Contato do responsável", d.get("contatoConstrutora")),
        ("Data de entrega do prédio", format_date(d.get("entregaConstrutora"))),
        ("Prédio na garantia", yn(d.get("predioGarantia"))),
        ("Garantia válida até", format_date(d.get("garantiaAte"))),
    ])

    controles = d.get("controlesPortao") or []
    for i, controle in enumerate(controles, 1):
        add_section(story, f"CONTROLE DE PORTÃO {i}", [
            ("Código/identificação", controle.get("codigo")),
            ("Portão/local", controle.get("portao")),
        ])

    add_section(story, "15. OBSERVAÇÕES", [
        ("Informações complementares", d.get("obs")),
    ])

    def _pdf_header_footer(canvas, document):
        canvas.saveState()
        logo = _prepare_alt_logo()
        if logo:
            canvas.drawImage(str(logo), 36, A4[1] - 47, width=49, height=32, preserveAspectRatio=True, mask="auto")
        canvas.setStrokeColor(ALT_LINE); canvas.setLineWidth(0.6)
        canvas.line(36, A4[1] - 53, A4[0] - 36, A4[1] - 53)
        canvas.setFillColor(ALT_PURPLE_DARK); canvas.setFont("Helvetica-Bold", 9)
        canvas.drawRightString(A4[0] - 36, A4[1] - 35, "Dossiê Condominial")
        canvas.setFillColor(ALT_GRAY); canvas.setFont("Helvetica", 7.5)
        canvas.drawRightString(A4[0] - 36, A4[1] - 46, "Informações que geram tranquilidade")
        canvas.setStrokeColor(ALT_LINE); canvas.line(36, 31, A4[0] - 36, 31)
        canvas.setFillColor(ALT_PURPLE_DARK); canvas.setFont("Helvetica-Bold", 8); canvas.drawString(36, 18, "alt")
        canvas.setFillColor(ALT_GRAY); canvas.setFont("Helvetica", 7.5); canvas.drawString(55, 18, "| Gestão de condomínios")
        canvas.drawRightString(A4[0] - 36, 18, f"{document.page:02d}")
        canvas.restoreState()

    doc.build(story, onFirstPage=_pdf_header_footer, onLaterPages=_pdf_header_footer)
    log_audit("pdf", f"PDF completo gerado para {name} por {session.get('username')}", session.get("username"))
    return send_file(
        path,
...
# ==========================================
# GESTÃO DE DEMANDAS E VENCIMENTOS
# ==========================================
...
def demandas_painel():
    ...
    for r in rows:
        item = dict(r)
        due_date = parse_date_value(item.get("data_vencimento"))
        if due_date:
            days_left = (due_date - today).days
            if item["status"] == "Concluída":
                item["status_prazo"] = "green"
                item["prazo_label"] = "Concluída"
            elif days_left < 0:
                item["status_prazo"] = "red"
                item["prazo_label"] = f"Vencida há {abs(days_left)} dia(s)"
                vencidas += 1
            elif days_left <= 7:
                item["status_prazo"] = "yellow"
                item["prazo_label"] = "Vence hoje" if days_left == 0 else f"Vence em {days_left} dia(s)"
                proximas += 1
            else:
                item["status_prazo"] = "green"
                item["prazo_label"] = f"Vence em {days_left} dias"
                em_dia += 1
        else:
            item["status_prazo"] = "green"
            item["prazo_label"] = "Sem prazo"

        demandas_formatadas.append(item)
...
def salvar_demanda():
    condominio_id = request.form.get("condominio_id", "").strip()
    titulo = clean(request.form.get("titulo", ""), 200)
    descricao = clean(request.form.get("descricao", ""), 1000)
    categoria = clean(request.form.get("categoria", "Geral"), 50)
    prioridade = clean(request.form.get("prioridade", "Média"), 20)
    data_vencimento = clean(request.form.get("data_vencimento", ""))
    responsavel = clean(request.form.get("responsavel", ""), 150)

    if not condominio_id or not titulo or not data_vencimento:
        flash("Condomínio, título e data de vencimento são obrigatórios.", "error")
        return redirect(url_for("demandas_painel"))

    try:
        datetime.strptime(data_vencimento, "%Y-%m-%d")
        condominio_id = int(condominio_id)
    except ValueError:
        flash("Data ou condomínio inválido.", "error")
        return redirect(url_for("demandas_painel"))

    with conn() as c:
        c.execute("""
            INSERT INTO demandas(condominio_id, titulo, descricao, categoria, prioridade, data_vencimento, status, responsavel, created_at)
            VALUES (?, ?, ?, ?, ?, ?, 'Pendente', ?, ?)
        """, (condominio_id, titulo, descricao, categoria, prioridade, data_vencimento, responsavel, datetime.now().isoformat(timespec="seconds")))
        c.commit()

    log_audit("criar_demanda", f"Demanda '{titulo}' criada por {session.get('username')}", session.get('username'))
    flash("Demanda registrada com sucesso.", "ok")
    return redirect(url_for("demandas_painel"))


@app.post("/demandas/<int:demanda_id>/status")
@login_required
def atualizar_status_demanda(demanda_id):
    novo_status = clean(request.form.get("status", "Pendente"), 30)
    with conn() as c:
        row = c.execute("SELECT titulo FROM demandas WHERE id = ?", (demanda_id,)).fetchone()
        if not row:
            flash("Demanda não encontrada.", "error")
            return redirect(url_for("demandas_painel"))
        c.execute("UPDATE demandas SET status = ? WHERE id = ?", (novo_status, demanda_id))
        c.commit()

    log_audit("status_demanda", f"Demanda #{demanda_id} alterada para '{novo_status}'", session.get('username'))
    flash(f"Status da demanda atualizado para {novo_status}.", "ok")
    return redirect(url_for("demandas_painel"))
...
def excluir_demanda(demanda_id):
    with conn() as c:
        row = c.execute("SELECT titulo FROM demandas WHERE id = ?", (demanda_id,)).fetchone()
        if row:
            c.execute("DELETE FROM demandas WHERE id = ?", (demanda_id,))
            c.commit()
            log_audit("excluir_demanda", f"Demanda '{row['titulo']}' excluída", session.get('username'))
            flash("Demanda excluída com sucesso.", "ok")
        else:
            flash("Demanda não encontrada.", "error")
